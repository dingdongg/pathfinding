...
export default class Controls extends Component<IControlsProps, IControlsState> {
    ...
    renderHeightChange() {
        return (<div className="adjust-size">
            <button className="up" onClick={() => this.setState((state) => { return { boardHeight: state.boardHeight + 1 } })}>^</button>
            <span>{this.state.boardHeight}</span>
            <button className="down" onClick={() => this.setState((state) => { return { boardHeight: state.boardHeight - 1 } })}>V</button>
        </div>)
    }

    renderWidthChange() {
        return (<div className="adjust-size">
            <button className="up" onClick={() => this.setState((state) => { return { boardWidth: state.boardWidth + 1 } })}>^</button>
            <span>{this.state.boardWidth}</span>
            <button className="down" onClick={() => this.setState((state) => { return { boardWidth: state.boardWidth - 1 } })}>V</button>
        </div>)
    }
